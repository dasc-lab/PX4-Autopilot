...
bool IndiControl::init()
{
  ...
  update_parameters();
	...
	ScheduleOnInterval(500_us); // 500 us interval, 2000 Hz rate
...
void IndiControl::compute_cmd_accel()
{
  ...
  _a_filt = a; // TODO: Update with the correct acceleration
...
void IndiControl::compute_cmd_quaternion()
{
  ...
  // EZRA, EQ:23
  Vector3f iz (0,0,1);
  Quatf xi_bar (iz, neg_bz_cmd_body); // see constructor in matrix/Quaternion.hpp
...
void IndiControl::compute_cmd_ang_accel()
{
  ...
  Vector3f ang_vel (_angular_velocity.xyz);
  Vector3f ang_vel_filt = ang_vel;
  ...
  _ang_accel_cmd = ang_accel_ref.zero_if_nan()
    ...
    + k_omega.emult ( (ang_vel_ref - ang_vel_filt).zero_if_nan() ); // why do i need the filtered version?
...
void IndiControl::compute_cmd_torque()
{
  ...
  _torque_filt = Vector3f(0,0,0); // TODO: FIX
